...
const Availability = () => {
  ...
  return (
    <div className="space-y-8 p-4 md:p-6">
      <h2 className="text-2xl font-bold">{t("availabilitySchedules")}</h2>
      ...
      <div className="bg-white p-6 rounded-xl border border-violet-600 shadow">
        ...
        >
          <select
            ...
            className="p-2 border border-violet-600 rounded bg-white"
          >
          ...
          <div className="relative" ref={dropdownRef}>
            <div
              ...
              className="p-2 border border-violet-600 rounded bg-white cursor-pointer flex justify-between items-center min-h-[42px]"
            >
              <span className="text-sm truncate">
                {selectedResourceIds.length > 0
                  ? `${selectedResourceIds.length} ${t("selected")}`
                  : t("selectResources")}
              </span>
              <ChevronDown size={16} />
            </div>
            {showResourceDropdown && (
              <div className="absolute z-10 w-full mt-1 bg-white border border-violet-600 rounded shadow-lg max-h-48 overflow-y-auto">
                {resources.map((r) => (
                  <div
                    key={r.id}
                    onClick={() => toggleResource(r.id)}
                    className="flex items-center p-2 hover:bg-violet-50 cursor-pointer text-sm"
                  >
                    <div
                      className={`w-4 h-4 border border-violet-600 rounded mr-2 flex items-center justify-center ${
                        selectedResourceIds.includes(String(r.id))
                          ? "bg-violet-600"
                          : "bg-white"
                      }`}
                    >
